...
with e1:
    ...
    chgbsc_id = st.text_input("Changeover BSC ID", value="9999")
with e2:
    cr_id = st.text_input("Process Room ID", value="115")
    cr_suit = st.text_input("Process Suite #", value="115")
with e3:
    chg_id = st.text_input("Changeover Room ID", value="116")
    chg_suit = st.text_input("Changeover Suite #", value="116")
...
# 5. Observations (Table 1)
st.subheader("5. Observations (Table 1)")
st.caption("Enter 'No Growth' for passing results. If there is growth, type the count (e.g., '3 CFU') and fill in ETX/ID.")

# Helper function to create row layout
def make_obs_row(label, prefix):
    c_obs, c_etx, c_id = st.columns([2, 1, 1])
    with c_obs:
        obs = st.text_input(f"{label} Obs", value="No Growth", key=f"{prefix}_obs")
    with c_etx:
        etx = st.text_area(f"{label} ETX", value="N/A", height=70, key=f"{prefix}_etx")
    with c_id:
        ids = st.text_area(f"{label} ID", value="N/A", height=70, key=f"{prefix}_id")
    return obs, etx, ids
...
# --- SMART LOGIC ---
st.markdown("### 6. Smart Narrative Generator")

# Initialize session state to hold the generated text
...
def run_smart_logic():
    # Definition Tuple: (Lowercase Key, User Obs, User ETX, User ID, Formal Name, Context Info)
    items = [
    ...
    if len(passed_daily) == 3:
        sent1 = "Upon reviewing the data presented in Table 1, no microbial growth was observed in personal sampling (left touch and right touch), surface sampling, or settling plates."
        ...
        sent1 = "Upon reviewing the data presented in Table 1, microbial growth was observed in all daily monitoring points."
...
if st.button("🪄 Auto-Generate Narrative & Details"):
    run_smart_logic()
...
def generate_file():
    ...
    context = {
        ...
        'changeover_name': change_name, 'changeover_initial': change_init,
        ...
        'changeover_id': chg_id, 'changeover_suit': chg_suit, 'changeoverbsc_id': chgbsc_id,
...
if uploaded_file:
    # Logic for filename: Ensure "OOS-" is present
    ...
    st.download_button(
        label="⬇️ Download Completed Report",
        data=generate_file(),
