...
if "temp_res" in st.session_state:
    ...
    with col1:
        ...
        # 🌟 这里只显示深度分析，不再显示细碎的 ADHD 步骤 🌟
        st.markdown(st.session_state.temp_res)
    
    with col2:
        st.subheader("✍️ 我的笔记与行动")
        user_thought = st.text_area("心得总结：", placeholder="用你的话说出这背后的“灵魂”...", height=100)
        
        st.write("🎯 **Action Items (AI 自动拆解，可修改)**")
        # 🌟 原子级步骤只出现在这里 🌟
            ...
            if user_thought:
                ...
                if sheet:
                    try:
                        ...
                        st.success("✅ 存入成功！知识已内化，行动已就位。")
                        del st.session_state.temp_res
                        st.rerun()
                    except Exception as e:
                        st.error(f"写入失败: {e}")
            else:
                st.warning("写下一句你的心得，哪怕只有几个字。")
