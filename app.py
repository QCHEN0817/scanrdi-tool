...
if selection == "ScanRDI":
    ...
    with p1:
        ...
        data["prepper_initial"] = st.text_input("✍️ Prepper Init")
    with p2:
        data["analyst_name"] = st.text_input("👤 Processor Name")
        data["analyst_initial"] = st.text_input("✍️ Processor Init")
    with p3:
        data["changeover_name"] = st.text_input("👤 Changeover Name")
        data["changeover_initial"] = st.text_input("✍️ Changeover Init")
    with p4:
        data["reader_name"] = st.text_input("👤 Reader Name")
        data["reader_initial"] = st.text_input("✍️ Reader Init")
    ...
    with e1:
        proc_bsc = st.selectbox("🖥️ Select Processing BSC ID", bsc_list)
        data["bsc_id"] = proc_bsc if proc_bsc != "Other" else st.text_input("🖥️ Manual Proc BSC")
    ...
    with e2:
        chg_bsc = st.selectbox("🖥️ Select Changeover BSC ID", bsc_list)
        data["chgbsc_id"] = chg_bsc if chg_bsc != "Other" else st.text_input("🖥️ Manual CHG BSC")
    ...
    with f1:
        ...
        data["scan_id"] = st.selectbox("🖥️ ScanRDI ID", scan_id_options)
    ...
    st.subheader("🗓️ Table 1: EM Observations")
    ...
    with wk1:
        data["obs_air_wk_of"] = st.text_input("🌬️ Weekly Air Obs", "No Growth")
        data["etx_air_wk_of"] = st.text_input("🆔 Air ETX #", "N/A")
        data["id_air_wk_of"] = st.text_input("🧬 Air ID", "N/A")
        data["weekly_initial"] = st.text_input("✍️ Weekly Monitor Init")
    with wk2:
    ...
    if st.button("🪄 Auto-Generate Draft Narratives"):
    ...
    data["equipment_summary"] = st.text_area("🔧 Equipment Summary (Editable)", value=st.session_state.equipment_summary, height=100)
    data["narrative_summary"] = st.text_area("📝 Narrative Summary (Editable)", value=st.session_state.narrative_summary, height=100)
